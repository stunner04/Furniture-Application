...
class UserAccountViewModel @Inject constructor(
    ...
    app: Application // getting the content resolver and using the AndroidViewModel
    ...
    // Initialized the screen with the user data from firestore
    init {
        getUser()
    }

    // The stored data is getting fetched from firestore
    fun getUser() {
    ...
    // Updating the user profile pic by validating the email and name of the user.Either with old pic or New pic.
    fun updateUser(user: User, imageUri: Uri?) {
        ...
        if (imageUri == null) {
            // if the new image is not given then the old image is retrieved from the firestore
            saveUserInformation(user, true)
        } else {
            // if the new image is  given then the old image is replaced with new (image)imageUri
